  ...
  return (
    <div className="bg-card border border-border shadow-sm rounded-2xl overflow-hidden mt-8">
      <div className="p-6 border-b border-border bg-muted flex items-center gap-2">
        ...
        <h2 className="text-lg font-medium text-foreground">Guest List ({users.length})</h2>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="p-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Total Bookings</th>
              <th className="p-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Joined Date</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            ...
            {users.map((u) => (
              <React.Fragment key={u.id}>
                ...
                >
                  <td className="p-4 text-muted-foreground group-hover:text-gold transition-colors">
                    {expandedRow === u.id ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                  </td>
                  <td className="p-4 font-medium text-foreground">{u.firstName} {u.lastName}</td>
                  <td className="p-4 text-muted-foreground">{u.email}</td>
                  <td className="p-4 text-foreground font-bold">{u.bookings.length}</td>
                  <td suppressHydrationWarning className="p-4 text-sm text-muted-foreground">{new Date(u.createdAt).toLocaleDateString()}</td>
                </tr>
                ...
                {expandedRow === u.id && (
                  <tr className="bg-white/[0.02]">
                    <td colSpan={5} className="p-0">
                      <div className="p-6 border-l-2 border-gold ml-10 my-4 bg-card rounded-r-lg">
                        ...
                        {u.bookings.length === 0 ? (
                          ...
                          <div className="space-y-4">
                            {u.bookings.map((booking: any) => (
                              <div key={booking.id} className="flex flex-col border-b border-border pb-4 last:border-0 last:pb-0">
                                <div className="flex justify-between items-center">
                                  <div>
                                    <div className="font-medium text-foreground">{booking.villa?.name || 'Unknown Property'}</div>
                                  ...
                                  <div className="mt-3 p-3 bg-muted rounded-lg border border-border">
                                    <div className="flex items-center gap-1 mb-1">
                                      {Array.from({ length: 5 }).map((_, i) => (
                                        <svg key={i} className={`w-3 h-3 ${i < booking.reviews[0].rating ? 'text-gold' : 'text-muted-foreground'}`} fill="currentColor" viewBox="0 0 20 20">
                                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                        </svg>
                                      ))}
                                    </div>
                                    <p className="text-sm text-muted-foreground italic">"{booking.reviews[0].comment}"</p>
                                  </div>
                                )}
                              </div>
                            ))}
