...
export default async function CustomerBookingDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6 p-8 bg-background min-h-screen text-foreground animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <Link href="/bookings" className="text-primary hover:underline text-sm mb-4 inline-block">&larr; Back to My Bookings</Link>
          <h1 className="text-3xl font-serif text-foreground">Booking Details</h1>
          <p className="text-muted-foreground font-mono mt-1">ID: {booking.bookingCode}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="border border-border bg-card shadow-sm rounded-xl p-6">
            <h2 className="text-xl font-bold text-primary mb-4">Stay Information</h2>
            <div className="grid grid-cols-2 gap-6">
              <div>
                <label className="text-xs text-muted-foreground uppercase block mb-1">Villa</label>
                <div className="font-medium text-foreground">{booking.villa.name}</div>
              </div>
              <div>
                <label className="text-xs text-muted-foreground uppercase block mb-1">Guests</label>
                <div className="font-medium text-foreground">{booking.totalGuests}</div>
              </div>
              <div className="col-span-2">
                <label className="text-xs text-muted-foreground uppercase block mb-1">Stay Dates</label>
          ...
          {booking.services.length > 0 && (
            <div className="border border-border bg-card shadow-sm rounded-xl p-6">
              <h2 className="text-xl font-bold text-primary mb-4">Added Services</h2>
              <div className="space-y-3">
                {booking.services.map(svc => (
                  <div key={svc.id} className="flex justify-between items-center p-3 rounded-lg bg-muted border border-border">
                    <div>
                      <div className="font-medium text-foreground">{svc.name}</div>
                      <div className="text-xs text-muted-foreground">Qty: {svc.quantity}</div>
                    </div>
                    <div className="font-bold text-foreground">₹{Number(svc.totalPrice).toLocaleString()}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="border border-border bg-card shadow-sm rounded-xl p-6 overflow-hidden">
                ...
                <table className="w-full text-left border-collapse min-w-[1200px]">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="p-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Edit Time</th>
                      <th className="p-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Action & Role</th>
                      <th className="p-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">State Change</th>
                      <th className="p-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Check In/Out</th>
                      <th className="p-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Guests</th>
                      ...
                      return (
                        <>
                          ...
                          <tr className="bg-muted/50 border-t border-border">
                            <td colSpan={10} className="p-4 text-right font-bold text-muted-foreground uppercase tracking-widest text-xs">
                              Totals
                            </td>
        ...
        <div className="space-y-6">
          <div className="border border-border bg-card shadow-sm rounded-xl p-6">
            <h2 className="text-xl font-bold text-primary mb-4">Price Breakdown</h2>
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Accommodation</span>
                <span className="text-foreground font-medium">₹{(Number(booking.currentTotal) - Number(booking.gstAmount) - Number(booking.cleaningFee) + Number(booking.discountAmount)).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="border-t border-border pt-3 mt-3 flex justify-between">
                ...
                <span className="text-primary font-bold text-lg">₹{Number(booking.currentTotal).toLocaleString()}</span>
              </div>
          ...
          <div className="border border-gold/20 bg-gold/5 rounded-xl p-6 space-y-4">
            ...
            <div className="space-y-2 text-xs text-muted-foreground">
              <div className="flex justify-between p-2 rounded bg-muted border border-border">
            ...
            {booking.cancellationRefund !== null && (
              <div className="pt-2 border-t border-border text-xs">
                <span className="text-muted-foreground">Refund Amount Processed: </span>
