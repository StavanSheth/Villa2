...
export async function POST(req: Request) {
  try {
    ...
    if (promoCode) {
      ...
      if (resolvedPromo) {
        if (resolvedPromo.status !== 'ACTIVE') {
          return NextResponse.json({ error: 'Promo code is no longer active' }, { status: 400 });
        }
        if (resolvedPromo.startDate && new Date() < resolvedPromo.startDate) {
        ...
        const userUsageCount = await prisma.promoUsage.count({
          where: { promoId: resolvedPromo.id, customerId: user.id }
        });
        if (userUsageCount >= (resolvedPromo.usagePerUser || 1)) {
          return NextResponse.json({ error: 'You have already reached the maximum usage limit for this promo code' }, { status: 400 });
        }
      }
    }
    ...
    if (segments && Array.isArray(segments)) {
      ...
      if (resolvedPromo) {
        if (resolvedPromo.type === 'PERCENTAGE') {
          advDiscount = bookingSubtotal * (Number(resolvedPromo.value) / 100);
          if (resolvedPromo.maxDiscount && advDiscount > Number(resolvedPromo.maxDiscount)) {
            advDiscount = Number(resolvedPromo.maxDiscount);
          }
        } else {
          advDiscount = Math.min(Number(resolvedPromo.value), bookingSubtotal);
        }
      }
      discount = advDiscount;
