...
export default function Preloader({ onComplete }: PreloaderProps) {
  ...
  useEffect(() => {
    ...
    const duration = 4000; // Minimum 4 seconds loading duration as requested
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-xs sm:max-w-md px-6 mt-4 flex flex-col items-center gap-3 z-10">
        <div className="flex items-center justify-between w-full text-xs font-serif uppercase tracking-[0.25em] text-[#911717]">
          <span>Chunawala&apos;s Seven C Villa</span>
          <span className="font-semibold text-black">{progress}%</span>
        </div>

        {/* Loading Line 0-100% */}
        <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden shadow-inner border border-gray-300">
          <div
        ...
        <span className="text-[10px] uppercase tracking-[0.3em] text-gray-500 font-light mt-1">
          Preparing Your Sanctuary Experience
        </span>
      </div>
    </div>
  );
