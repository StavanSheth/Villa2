  ...
  return (
    <div className="flex h-screen overflow-hidden transition-colors duration-1000 relative bg-transparent">
      ...
      <aside 
        ...
        transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] flex flex-col liquid-glass border-y-0 border-l-0`}
      >
        <div className="h-16 flex items-center justify-center border-b border-white/10 dark:border-black/10">
          <div className="flex items-center gap-2 px-2 overflow-hidden whitespace-nowrap">
            ...
            {sidebarOpen && (
              <div className="flex flex-col animate-fade-in">
                <span className="font-serif text-lg tracking-wide text-[var(--text-dark)]">
                  {title === 'Mavon Dashboard' ? 'Chunawala\'s' : title}
                </span>
                <span className="text-[8px] uppercase tracking-[0.22em] text-gold font-medium -mt-1">
                  Seven C Villa
                </span>
              </div>
            )}
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {navItems.map((item, idx) => (
            <a 
              ...
              className="group flex items-center px-4 py-3 text-sm font-medium rounded-2xl transition-all duration-300 hover:bg-white/10 hover:text-gold text-[var(--text-sec-dark)]"
            >
              {item.icon ? (
                ...
                <span className="flex-shrink-0 w-5 h-5 mr-3 rounded-full bg-white/5 border border-white/10 group-hover:border-gold/50 transition-colors"></span>
              )}
        ...
        <div className="p-4 border-t border-white/10 dark:border-black/10 flex flex-col gap-4">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        {/* Top Navbar */}
        <header 
          className="h-16 flex items-center justify-between px-6 liquid-glass border-t-0 border-x-0 z-20"
        >
          <div className="flex items-center gap-4">
            <button 
              onClick={toggleSidebar}
              className="p-2 rounded-full hover:bg-white/10 transition-colors text-[var(--text-dark)]"
            >
            ...
            <div className="relative group hidden sm:block">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2 rounded-full text-sm transition-all duration-300 w-64 focus:w-80 bg-white/5 border border-white/10 focus:border-gold/50 text-[var(--text-dark)] focus:outline-none focus:ring-1 focus:ring-gold/30"
              />
            </div>
          </div>
          <div className="flex items-center space-x-4">
            {/* Notifications & Profile */}
            <button className="p-2 rounded-full hover:bg-white/10 transition-colors text-[var(--text-dark)] relative group">
            ...
            <div className="flex items-center space-x-3 pl-4 border-l border-white/10">
              ...
              <div className="hidden md:flex flex-col">
                <span className="text-sm font-medium text-[var(--text-dark)]">{userProfile?.name || 'User'}</span>
                <span className="text-[10px] uppercase tracking-wider text-gold">Dashboard</span>
              </div>
            </div>
          </div>
        </header>
